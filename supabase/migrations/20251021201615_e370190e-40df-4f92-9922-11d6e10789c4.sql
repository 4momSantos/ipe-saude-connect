-- Corrigir função RPC para evitar agregações aninhadas
...
BEGIN
  ...
  SELECT 
    ...
    -- Análise de critérios usando CTE para evitar agregações aninhadas
    (
      WITH criterios_media AS (
        SELECT 
          elem->>'criterio_nome' as nome,
          AVG((elem->>'pontuacao')::NUMERIC) as media
        FROM avaliacoes_prestadores avp2
        CROSS JOIN jsonb_array_elements(avp2.criterios) AS elem
