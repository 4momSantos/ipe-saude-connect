-- Criar função SQL para busca completa com prazos e OCR
...
BEGIN
  ...
  SELECT 
    ...
    CASE WHEN p_incluir_prazos THEN calcular_status_documento((dc.data_vencimento - CURRENT_DATE)::INTEGER) ELSE NULL END AS status_prazo,
    
    -- Campos de OCR
    CASE WHEN p_incluir_ocr THEN id_doc.ocr_resultado ELSE NULL END AS ocr_resultado,
    CASE WHEN p_incluir_ocr THEN COALESCE(id_doc.ocr_processado, FALSE) ELSE FALSE END AS ocr_processado,
