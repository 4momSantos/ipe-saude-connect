-- Corrigir loop de CRMs no trigger sync_approved_inscricao_to_credenciado
-- Remove loop e insere apenas UM registro de credenciado_crms
...
BEGIN
  ...
  IF NEW.status = 'aprovado' AND (OLD.status IS NULL OR OLD.status != 'aprovado') THEN
    ...
    -- PROCESSAR CRM E ESPECIALIDADES (APENAS UM REGISTRO)
    ...
    IF v_crm IS NOT NULL AND v_dados->'consultorio'->'especialidades_ids' IS NOT NULL THEN
      ...
      IF v_especialidade_id IS NOT NULL THEN
        ...
        -- Inserir UMA VEZ com ON CONFLICT
