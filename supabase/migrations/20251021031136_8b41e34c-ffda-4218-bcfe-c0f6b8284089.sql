-- Corrigir a função sync_credenciado_from_contract para buscar CRMs no local correto
...
BEGIN
  ...
  IF v_credenciado_id IS NOT NULL THEN
    UPDATE credenciados SET
      nome = v_dados_contrato->>'candidato_nome',
      cpf = v_dados_contrato->>'candidato_cpf',
      rg = v_dados_contrato->>'candidato_rg',
      email = v_dados_contrato->>'candidato_email',
      telefone = v_dados_contrato->>'candidato_telefone',
      celular = v_dados_contrato->>'candidato_celular',
      cep = v_dados_contrato->>'candidato_cep',
      endereco = v_dados_contrato->>'candidato_endereco_completo',
      cidade = v_dados_contrato->>'candidato_cidade',
      estado = v_dados_contrato->>'candidato_estado',
