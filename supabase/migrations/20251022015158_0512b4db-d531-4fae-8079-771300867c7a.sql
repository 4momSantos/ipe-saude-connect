-- Adicionar especialidades à função buscar_credenciados_com_documentos_completo
-- e habilitar realtime para credenciado_crms

-- Habilitar realtime na tabela credenciado_crms
ALTER TABLE credenciado_crms REPLICA IDENTITY FULL;
ALTER PUBLICATION supabase_realtime ADD TABLE credenciado_crms;

-- Recriar função com campo especialidades
...
BEGIN
  ...
  docs_por_credenciado AS (
    SELECT 
      ...
      -- ✅ Buscar especialidades do credenciado
      (
        SELECT jsonb_agg(
          jsonb_build_object(
            'crm_id', cc.id,
          ...
          ) ORDER BY em.nome ASC NULLS LAST
        )
        FROM credenciado_crms cc
        LEFT JOIN especialidades_medicas em ON cc.especialidade_id = em.id
        WHERE cc.credenciado_id = c.id
        ...
        jsonb_build_object(
          ...
          'dias_para_vencer', 
            CASE 
              ...
              THEN (df.data_vencimento - CURRENT_DATE)
              ELSE NULL
            END,
  ...
  SELECT 
    ...
    prox_venc,
    COALESCE(especialidades_json, '[]'::jsonb),
