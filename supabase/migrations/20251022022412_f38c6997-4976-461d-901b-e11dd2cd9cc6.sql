-- Corrigir coluna numero_credenciado na função buscar_credenciados_com_documentos_completo
...
BEGIN
  ...
  docs_unificados AS (
    ...
    SELECT 
      ...
      CASE 
        WHEN id_doc.data_vencimento IS NOT NULL THEN (id_doc.data_vencimento - CURRENT_DATE)
        ELSE NULL 
      END as dias_para_vencer
    FROM inscricao_documentos id_doc
    INNER JOIN credenciados_filtrados cf ON cf.inscricao_id = id_doc.inscricao_id
    WHERE id_doc.is_current = true
  ),
