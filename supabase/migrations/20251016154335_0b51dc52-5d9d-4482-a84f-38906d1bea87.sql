-- Modificar função para incluir todos os filtros de status de credenciamento
...
BEGIN
      ...
      CASE 
        WHEN p_incluir_prazos THEN 
          ...
          WHERE p.credenciado_id = dc.credenciado_id
            AND p.entidade_tipo = 'documento'
            AND p.entidade_id = dc.id::TEXT)
        ELSE NULL
      END AS prazos,
      CASE 
  ...
  documentos_inscricoes_filtrados AS (
    -- Documentos de não-credenciados (se solicitado)
    SELECT 
      id.id,
      id.inscricao_id,
    ...
    WHERE p_incluir_nao_credenciados = TRUE
      AND c.id IS NULL  -- Apenas inscrições sem credenciado
