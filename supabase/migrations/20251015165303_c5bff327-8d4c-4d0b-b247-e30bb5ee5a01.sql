-- Atualizar função sync_approved_inscricao_to_credenciado para incluir datas
...
BEGIN
  IF NEW.status = 'aprovado' AND (OLD.status IS NULL OR OLD.status != 'aprovado') THEN
    ...
    -- CRIAR CREDENCIADO COM DATAS
    INSERT INTO public.credenciados (
      inscricao_id, nome, cpf, cnpj, rg, data_nascimento,
      email, telefone, celular, endereco, cidade, estado, cep,
      status, observacoes,
      data_solicitacao, data_habilitacao, data_inicio_atendimento
    )
    VALUES (
      NEW.id, v_nome,
      ...
      v_email, v_telefone, v_celular, v_endereco_completo,
      ...
      v_data_solicitacao, NOW(), CURRENT_DATE
    )
