...
export async function seedEspecialidades(
  ...
  console.log(`[SEED-ESPECIALIDADES] Verificando ${count} especialidades (dry run: ${dryRun})`);
  ...
  if (existingCount && existingCount >= count) {
    console.log(`[SEED-ESPECIALIDADES] ${existingCount} especialidades já existem`);
    return { success: true, phase: 'especialidades', created: 0, errors: [], duration: 0 };
  }

  if (dryRun) {
    return { success: true, phase: 'especialidades', created: count - (existingCount || 0), errors: [], duration: 0 };
  }
  ...
  const toCreate = especialidadesRS
  ...
  const { data, error } = await supabase
    .from('especialidades_medicas')
    .insert(toCreate.map(esp => ({ codigo: esp.codigo, nome: esp.nome, ativa: true })))
    .select();

  if (error && !error.message.includes('duplicate')) {
    throw new Error(`Erro ao criar especialidades: ${error.message}`);
  }

  const created = data?.length || 0;
  console.log(`[SEED-ESPECIALIDADES] ${created} especialidades criadas`);
  
  return { success: true, phase: 'especialidades', created, errors: [], duration: 0 };
}
