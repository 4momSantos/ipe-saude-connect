...
serve(async (req) => {
  ...
  try {
    ...
    // Aplicar filtros de busca nos campos principais
    if (termo) {
      query = query.or(`tipo_documento.ilike.*${termo}*,descricao.ilike.*${termo}*,numero_documento.ilike.*${termo}*,observacao.ilike.*${termo}*`);
    }
    if (status) {
